...
public class StatusBarUtil {
    ...
    public static void setColor(Activity activity, String colorValue) {
        if (!colorValue.startsWith("#")) {
            return;
        }
        StatusBarUtil.setTransparent(activity);
        if (colorValue.endsWith("#")) {     //如果以#结尾，则改变状态栏字体和图标颜色为深色【默认为白色，在白色背景下不显示】
            setStatusBarTextColorStyle(activity, true);
            colorValue = colorValue.substring(0, colorValue.lastIndexOf("#"));
        }
        setColor(activity, Color.parseColor(colorValue), 0);
    }

    /**
     * 设置当前activity状态栏的字体颜色
     *
    ...
    public static void setColor(Activity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int statusBarAlpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            addFakeStatusBarView(activity);
        }
    }

    public static void addFakeStatusBarView(Activity activity) {
...
//            fakeStatusBarView.setBackgroundColor(calculateStatusColor(color, statusBarAlpha));
            fakeStatusBarView.setBackgroundResource(R.drawable.ic_launcher_background);
        } else {
            decorView.addView(createPicStatusBar(activity));
        }
    ...
    public static void setTranslucentForImageView(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha,
        ...
        if (needOffsetView != null) {
            ...
            if (needOffsetView instanceof ViewGroup) {
                ViewGroup.MarginLayoutParams layoutParams = (ViewGroup.MarginLayoutParams) needOffsetView.getLayoutParams();
                layoutParams.setMargins(layoutParams.leftMargin, layoutParams.topMargin + getStatusBarHeight(activity),
                        layoutParams.rightMargin, layoutParams.bottomMargin);
            } else if (needOffsetView instanceof View) {
                LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) needOffsetView.getLayoutParams();
                if (params != null) {
                    ...
                    params.topMargin += getStatusBarHeight(activity);
                    needOffsetView.setLayoutParams(params);
                    Log.e("zzzzzz", "EWEEEEEEEEEEEEEEEEEEEEE");
                }
            }
     ...
     * @return 状态栏矩形条
     */
    private static View createPicStatusBar(Activity activity) {
        // 绘制一个和状态栏一样高的矩形
        View statusBarView = new View(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
        statusBarView.setLayoutParams(params);
//        statusBarView.setBackgroundColor(calculateStatusColor(color, alpha));
        statusBarView.setBackgroundResource(R.drawable.ic_launcher_background);
        statusBarView.setId(FAKE_STATUS_BAR_VIEW_ID);
        return statusBarView;
    }
